...
class Neuron:
    ...
    def draw(self, neuron_radius, ax, color='lightblue'):
        ...
        circle = plt.Circle((self.x, self.y), radius=neuron_radius, fill=True, color=color)
        ax.add_patch(circle)
...
def visualize_network(network_structure, title='Neural Network Architecture', figsize=(12, 8), save_path=None):
    """
...
def visualize_mlp(mlp, title='MLP Architecture', figsize=(12, 8), save_path=None):
    ...
    network_structure = [len(layer.neurons[0].w) for layer in mlp.layers]  # Input sizes for each layer
